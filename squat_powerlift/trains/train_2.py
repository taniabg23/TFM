...
def save_history_csv(history, filepath="./csvs/training_history_2.csv"):
...
def save_learning_curves(history, filepath="./csvs/learning_curves_2.png"):
    try:
        import matplotlib.pyplot as plt
    except Exception:
        print("No se pudo generar learning_curves_2.png (matplotlib no disponible).")
        return
...
model = Sequential([
    ...
    LSTM(64),
    LayerNormalization(),   # 👈 cambio clave
    Dropout(0.3),
...
callbacks = [
    ...
    ModelCheckpoint(
        "./models/bests/best_squat_model_2.keras",
        monitor="val_loss",
        save_best_only=True,
        verbose=1
    )
...
save_history_csv(history, filepath="./csvs/training_history_2.csv")
save_learning_curves(history, filepath="./csvs/learning_curves_2.png")
...
model.save("./models/squat_model_final_2.h5")
model.save("./models/squat_model_final_2.keras")